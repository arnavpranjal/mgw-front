...
export default function Entities() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get<Stage[]>(Backend_URL+"/entity");
  ...
  const handleCreateRole = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (typeof role === "string" && role.trim() !== "") {
      try {
        await axios.post(Backend_URL+"/entity", newRole);
  ...
  const handleEditRole = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      try {
        await axios.put(Backend_URL+`/entity/${roleId}`, {
          displayName,
        });
  ...
  const handleDeleteRole = async (roleId: string | null) => {
    ...
    try {
      await axios.delete(Backend_URL+`/entity/${roleId}`);
  ...
  const handleReset = async () => {
    try {
      await axios.post(Backend_URL+"/entity/reset");
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.post(Backend_URL+"/entity/save", stagesToUpdate);
      fetchData();
      toast({
